...
return uploadPayload (self.db, containerName, data, name, db, encoding, mimetype)
.then((res) => {
  cb(null, res);
})
...
/*
  ...
  busboy.on('field', function (fieldname, val, fieldnameTruncated, valTruncated, encoding, mimetype) {
    ...
    busboy.extraFields = busboy.extraFields || {};
    busboy.extraFields[fieldname] = val;
  })
  ...
  busboy.on('file', function (fieldname, file, filename, encoding, mimetype) {
    ...
    var options = {
      ...
      filename: filename,
      metadata: _.merge(busboy.extraFields, {
    ...
    stream.on('error', (err) => {
      logger.log('error while stream file in busboy , ', err);
      cb(err);
    });
